...
# Find all function definitions with their character positions
...
for i, match in enumerate(matches):
    ...
    if func_name.startswith('use'):
        target_file = f"/Users/javierlazo/Desktop/Proyecto Curbe/client/src/components/sections/respond-style/hooks.ts"
        # For hooks, we already added them to hooks.ts, so skip
        print(f"  -> SKIPPED (already in hooks.ts)")
        continue
    
    # Check if file already exists
    import re to get the actual component name
